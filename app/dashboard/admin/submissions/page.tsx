      ...
      <div className="space-y-4">
        {submissions && submissions.length > 0 ? (
          submissions.map((submission: Submission) => (
            <div
              key={submission.id}
              className={`rounded-xl border-2 bg-main dark:bg-surface p-6 ${submission.status === 'pending'
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted dark:text-muted">
                    <span>
                  ...
                  <div className="mt-4 flex gap-2">
                    ...
                    {submission.github_repo_url && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 rounded-lg bg-gray-900/90 dark:bg-main/10 px-3 py-2 text-sm font-medium text-text-main hover:bg-gray-800 dark:hover:bg-main/20 transition-colors"
                      >
                    ...
                    <Link
                      href={`/dashboard/quests/${submission.course_exercises.courses.id}`}
                      className="inline-flex items-center gap-2 rounded-lg bg-sidebar-border/50 px-3 py-2 text-sm font-medium text-muted dark:text-muted hover:bg-surface dark:hover:bg-sidebar-border transition-colors"
                    >
                      Ver curso
                    </Link>
                  </div>
                ...
                {submission.status === 'pending' && (
                  <div className="ml-4 flex gap-2">
                    <form action={`/api/submissions/${submission.id}/approve`} method="POST">
                      <button
                        type="submit"
                        className="rounded-lg bg-green-600 px-4 py-2 text-sm font-medium text-text-main dark:text-text-main hover:bg-green-700 transition-colors"
                      >
                        ✓ Aprobar
                      </button>
                    </form>
                    <form action={`/api/submissions/${submission.id}/reject`} method="POST">
                      <button
                        type="submit"
                        className="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-text-main dark:text-text-main hover:bg-red-700 transition-colors"
                      >
                        ✗ Rechazar
                      </button>
                    </form>
          ...
          <div className="rounded-xl bg-main dark:bg-surface p-12 text-center border border-border dark:border-border">
            <p className="text-muted dark:text-muted">No hay submissions para revisar.</p>
          </div>
        )}
      </div>
    </>
